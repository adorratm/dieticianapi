...
namespace App\Http\Controllers\Api\Dietician\Datatables;
...
class indexController extends Controller
{
    ...
    public function getBySearch(Request $request)
    {
        if (empty($request->search) || $request->search == "null") {
            return Redirect::to(route("dietician.datatables.index", "table={$request->table}&per_page={$request->per_page}&where_column={$request->where_column}&where_value={$request->where_value}"));
        }
    ...
    public function isCoverSetter(Request $request)
    {
        ...
        if (!empty($data)) {
            ...
            if ($enable) {
                ...
                return response()->json(["success" => false, "title" => "Başarısız!", "msg" => "Durum Değiştirilemedi.2"]);
            }
        } else {
            return response()->json(["success" => false, "title" => "Başarısız!", "msg" => "Durum Değiştirilemedi.1"]);
        }
    }
